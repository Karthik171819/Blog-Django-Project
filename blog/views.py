...
from .forms import ContactForm, ForgotPasswordForm, RegisterForm, LoginForm, ResetPasswordForm, PostForm
...
def detail(request, slug):
    ...
    try:
        #getting data by post_id
        post = Post.objects.get(slug=slug)
        related_posts = Post.objects.filter(category = post.category).exclude(pk=post.id) #its filter the based on category
...
def about_view(request):
    ...
    if about_content is None or not about_content.content:
        about_content = "Default content goes here or admin wants to update" #default text shows when content was deleted
    else:
...
def publish_post(request, post_id):
    ...
    post.save()
    messages.success(request, "Successfully Post Published")
    return redirect("blog:dashboard")
